...
function showOnboarding(): void {
  ...
  overlay.querySelector<HTMLButtonElement>('#onboardDone')!.addEventListener('click', done);
  overlay.addEventListener('mousedown', (event: MouseEvent) => { if (event.target === overlay) done(); });
}

function maybeShowOnboarding(): void {
  if (!localStorage.getItem('sdOnboarded')) showOnboarding();
}
