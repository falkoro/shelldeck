...
let privateSessions = new Set<string>();
...
function sessionWorkTitle(session: string): string {
  if (isPrivateSession(session)) return '';
...
function clearShellAutoTitleCache(name?: string): void {
  try {
    if (!name) {
...
function shellboxTitle(name: string): string {
  if (isPrivateSession(name)) return '';
...
function shellAutoDisplayLabel(name: string, fallback: string): string {
  ...
  if (!generated || generated.toLowerCase() === base.toLowerCase()) return base;
  return `${base} · ${generated}`;
}

function shellRawNameBadge(name: string, displayLabel: string): string {
  if (name === 'main' || /^slot\d+$/.test(name)) return '';
...
function isPrivateSession(name: string): boolean {
  return privateSessions.has(name) || Boolean(shellPreviewByName(name)?.private);
}

function setPrivateSessionState(name: string, isPrivate: boolean): void {
  if (!name) return;
  if (isPrivate) privateSessions.add(name);
  else privateSessions.delete(name);
  latestShells = latestShells.map((shell) => shell.name === name ? { ...shell, private: isPrivate, output: isPrivate ? '' : shell.output } : shell);
  if (isPrivate) clearShellAutoTitleCache(name);
  shellTabsSignature = '';
  updatePrivateUi();
}

function applyPrivateSessions(names: string[]): void {
...
function updatePrivateUi(): void {
  document.querySelectorAll<HTMLButtonElement>('[data-private-shell]').forEach((button) => {
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-shellin]').forEach((button) => {
...
(window as any).updatePrivateUi = updatePrivateUi;
...
function removeShellPreviewCache(name: string): void {
  try {
    const cached = storageJson<{ savedAt?: number; shells?: ShellPreview[] }>(SHELL_PREVIEW_CACHE_KEY, {});
    if (!Array.isArray(cached.shells)) return;
...
function stopReady(name: string): boolean {
  const session = sessionByName(name);
...
function updateUnlockState(): void {
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-private-shell],[data-wipe-shell]').forEach((button) => {
  ...
  const newSession = document.getElementById('newSessionBtn') as HTMLButtonElement | null;
  if (newSession) newSession.disabled = !shellUnlocked;
