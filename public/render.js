...
function createShellCard(shell) {
  ...
  <pre data-role="output"></pre>`;
    ...
    article.querySelector('[data-clear-preview]').dataset.clearPreview = shell.name;
    article.querySelector('[data-minimize-shell]').dataset.minimizeShell = shell.name;
    article.querySelector('[data-resize-preview]').dataset.resizePreview = shell.name;
...
function updateShellCard(card, shell) {
    ...
    if (clearedOutputs[shell.name] && clearedOutputs[shell.name] === shell.output)
        output = '';
    if (clearedOutputs[shell.name] && clearedOutputs[shell.name] !== shell.output)
        delete clearedOutputs[shell.name];
...
function markSelectedShell() {
    document.querySelectorAll('.terminal-card,.session-item').forEach((item) => {
...
function applyWorkTitles() {
    document.querySelectorAll('[data-shell-card]').forEach((card) => setWorkTitle(card, card.dataset.shellCard || ''));
}
