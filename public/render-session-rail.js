...
function familyRailGlyph(family, badge) {
    const key = (family || '').toLowerCase();
    const name = (badge || '').toLowerCase();
    if (key === 'agent' || name === 'cx' || name === 'cd')
        return icon('bot');
    if (name === 'cl' || name === 'cc')
        return icon('bot');
...
function renderSessionRailItem(session, opts) {
    const state = sessionRuntime(session);
...
function renderSessionRail() {
    ...
    const groupBlock = (key, label, items, draggable) => {
        if (!items.length)
            return '';
...
function bindPinDrag(rail) {
    let dragName = '';
    rail.querySelectorAll('[data-pin-drag]').forEach((el) => {
...
function markSessionRailSelected() {
    document.querySelectorAll('[data-session-rail-item]').forEach((item) => {
...
function applySessionRailOpen() {
    ...
    if (closeBtn)
        closeBtn.hidden = window.innerWidth > 760;
}
