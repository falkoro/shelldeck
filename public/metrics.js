...
const RED_CONTAINER_STATES = new Set(['unhealthy', 'restarting', 'crashed']);
...
function containerState(status) {
    ...
    if (exit)
        return exit[1] === '0' ? 'stopped' : 'crashed';
    if (s.startsWith('up'))
        return 'running';
    return 'stopped';
}
...
function statChip(kind, fillPct, text, title) {
...
function containerStatChipsHtml(c) {
    const chips = [];
    const cpu = parsePercent(c.cpu);
    if (cpu !== null)
        chips.push(statChip('cpu', cpu, `CPU ${cpu.toFixed(cpu < 10 ? 1 : 0)}%`, `CPU ${c.cpu}`));
    if (c.mem) {
...
async function openFixModal(host, engine, name) {
    if (document.getElementById('fixModal'))
        return;
    ...
    const ack = overlay.querySelector('#fixAck');
    const confirm = overlay.querySelector('#fixConfirm');
    ...
    overlay.querySelector('form')?.addEventListener('submit', (event) => {
        ...
        const agentId = overlay.querySelector('#fixAgentSelect')?.value || '';
